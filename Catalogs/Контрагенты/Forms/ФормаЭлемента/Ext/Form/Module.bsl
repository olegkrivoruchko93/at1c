...
Процедура иб_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если Константы.ИБ_ИспользоватьЗаполнениеКонтрагентаDadata.Получить() Тогда
		ДобавитьКнопкуЗаполнитьРеквизитыПоДаннымDaData();
	КонецЕсли;
	
	ИБ_ИспользоватьПодсказкуФИОDadata = Константы.ИБ_ИспользоватьПодсказкуФИОDadata.Получить();
	ИБ_ИспользоватьПодсказкуEmailDadata = Константы.ИБ_ИспользоватьПодсказкуEmailDadata.Получить();
	
КонецПроцедуры

&НаКлиенте
Процедура иб_ПередЗаписьюПеред(Отказ, ПараметрыЗаписи)
...
&НаКлиенте
Процедура иб_КонтактнаяИнформацияПолеEmailКонтрагентыАвтоПодборПосле(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура иб_ФИООсновноеКонтактноеЛицоАвтоПодборПосле(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ЗаполнитьРеквизитыПоДаннымDaData()
	...
	Если ЭтоИНН(ПолеПоискаИНН) Тогда
		...
		suggestion = ИБ_DaData.СведенияОЮридическомЛицеПоИНН(ПолеПоискаИНН);
		...
		ИначеЕсли suggestion.Количество() = 1 Тогда
			ЗаполнитьРеквизитыКонтрагентаПоСтруктуре(suggestion.suggestions[0]);
		Иначе
			СписокВыбора = СформироватьСписокВыбора(suggestion.suggestions);
			СписокВыбора.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("ПослеВыбораКонтрагента", ЭтотОбъект), "По данному ИНН найдено несколько контрагентов, выберите нужного:",);
		КонецЕсли;
...
&НаКлиенте
Процедура иб_ЗаписатьДубльПосле(Команда)
...
&НаСервере
Процедура ДобавитьКнопкуЗаполнитьРеквизитыПоДаннымDaData()
	
	НоваяКоманда = ЭтаФорма.Команды.Добавить("ЗаполнитьРеквизитыПоДаннымDaData");
	НоваяКоманда.Действие = "ЗаполнитьРеквизитыПоДаннымDaData";
	НоваяКоманда.Заголовок = "Заполнить (DaData)";
	
	НоваяКнопка = ЭтаФорма.Элементы.Добавить("ЗаполнитьРеквизитыПоДаннымDaData", Тип("КнопкаФормы"), ЭтаФорма.Элементы.ГруппаИНН);
	НоваяКнопка.Отображение = ОтображениеКнопки.КартинкаИТекст;
	НоваяКнопка.Картинка = БиблиотекаКартинок.иб_DaData;
	НоваяКнопка.ИмяКоманды = "ЗаполнитьРеквизитыПоДаннымDaData";
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВыбораКонтрагента(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйЭлемент <> Неопределено Тогда
		ЗаполнитьРеквизитыКонтрагентаПоСтруктуре(ВыбранныйЭлемент.Значение);
	КонецЕсли;
...
//
&НаКлиенте
Процедура ЗаполнитьРеквизитыКонтрагентаПоСтруктуре(ДанныеЗаполнения)
	...
	ЗаполнитьАдресНаСервере(ДанныеЗаполнения);
...
&НаСервере
Процедура ЗаполнитьАдресНаСервере(ДанныеЗаполнения)
...
Функция СформироватьСписокВыбора(suggestions)
