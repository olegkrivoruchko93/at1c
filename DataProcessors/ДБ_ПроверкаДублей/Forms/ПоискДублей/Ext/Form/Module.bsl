...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КлючОбъекта  = "Обработки.ДБ_ПроверкаДублей";
	КлючНастроек = "Обработки.ДБ_ПроверкаДублей";
	ОписаниеНастроек = "Настройки проверки дублей";
	Пользователь = "Общие";

	НастройкиФормы = ХранилищеОбщихНастроек.Загрузить(КлючОбъекта, КлючНастроек, ОписаниеНастроек, Пользователь);
...
Функция НастройкиПоискаУдаленияДублей()
	...
	Результат.Вставить("ДействиеПослеПроверкиДублей", Перечисления.ДБ_ДействиеПослеПроверкиДублей.ПустаяСсылка());
	Результат.Вставить("НаименованиеНастройки", "");
...
Процедура ПриСозданииНаСервереИнициализацияДанных(НастройкиФормы)
	...
	//+kriv, 17.11.2023
	ДействиеПослеПроверкиДублей = НастройкиФормы.ДействиеПослеПроверкиДублей;
	НаименованиеНастройки = НастройкиФормы.НаименованиеНастройки;
	//+/kriv@, 17.11.2023
...
Процедура СохранитьПользовательскиеНастройки()
	...
	НастройкиФормы.Вставить("НаименованиеНастройки", НаименованиеНастройки);
	...
	НастройкиФормы.Вставить("ДействиеПослеПроверкиДублей", ДействиеПослеПроверкиДублей);
	...
	ОписаниеНастроек = "Настройки проверки дублей";
	Пользователь = "Общие";
	ХранилищеОбщихНастроек.Сохранить(КлючОбъекта,  КлючНастроек, НастройкиФормы, ОписаниеНастроек, Пользователь);
...
Процедура СохранитьНастройкиНаСервере()
	...
	Хранилище = Новый ХранилищеЗначения(НастройкиФормы);
	...
	МенеджерЗаписи = РегистрыСведений.ДБ_НастройкиПроверкиДублей.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.ОбластьПоискаДублей = ОбластьПоискаДублей;
	МенеджерЗаписи.НаименованиеНастройки = НаименованиеНастройки;
	МенеджерЗаписи.Настройка = Хранилище;
...
&НаСервере
Процедура ПрочитатьНастройкиНаСервере(ВыбраннаяОрганизация, ВыбраннаяОбластьПоискаДублей)
	...
	|ГДЕ
	|	ДБ_НастройкиПроверкиДублей.НаименованиеНастройки = &НаименованиеНастройки
	|	И ДБ_НастройкиПроверкиДублей.ОбластьПоискаДублей = &ОбластьПоискаДублей";
	
	Запрос.УстановитьПараметр("ОбластьПоискаДублей", ВыбраннаяОбластьПоискаДублей);
	Запрос.УстановитьПараметр("НаименованиеНастройки", НаименованиеНастройки);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если РезультатЗапроса.Пустой() Тогда
		//ToDo: Добавить предупреждение пользователю
		Возврат;
	КонецЕсли;
	...
	НастройкиФормы = Выборка.Настройка.Получить();
	
	ПовторяемДействияПриОткрытииФормы(НастройкиФормы);
	
КонецПроцедуры

//ToDo: Дать более подходящее название и может убрать лишнее
&НаСервере
Процедура ПовторяемДействияПриОткрытииФормы(НастройкиФормы)
...
Процедура ПрочитатьНастройки(Команда)
	ПрочитатьНастройкиНаСервере(Элементы.НастройкиПроверкиДублей.ТекущиеДанные.НаименованиеНастройки,
								   Элементы.НастройкиПроверкиДублей.ТекущиеДанные.ОбластьПоискаДублей);
	ПриАктивацииШагаМастера()
КонецПроцедуры
...
Функция КлючОбъектаНастроек()
	...
	Возврат "Обработки.ДБ_ПроверкаДублей";
	
КонецФункции // КлючОбъектаНастроек()
