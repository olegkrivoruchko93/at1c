...
Процедура ДБ_ВыполнитьПроверку(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок)
	...
	Если СтрокаПроверки.Идентификатор = "СозданиеУведомленийЕНС" Тогда
		Результат = ПроверитьПравильностьСозданныхУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок);
	ИначеЕсли СтрокаПроверки.Идентификатор = "СтатусУведомленийЕНС" Тогда
		Результат = ПроверитьСтатусУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок);
	ИначеЕсли СтрокаПроверки.Идентификатор = "СуммыУведомленийЕНС" Тогда
		Результат = ПроверитьСуммыУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок);
	КонецЕсли;
...
#Вставка
	...
	Если СтрокаПроверки.Идентификатор = "ОтрицательныеОстаткиНаСчетахБУ" Тогда
		Результат = ДБ_ПроверитьОтрицательныеОстаткиНаСчетахБУ(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета10" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета10(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета19" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета19(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета41" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета41(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета60" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета60(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета62" Тогда                               
		Результат = ДБ_ПроверитьОстаткиСчета62(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета6802" Тогда                               
		Результат = ДБ_ПроверитьОстаткиСчета6802(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета76АВ" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета76АВ(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиСчета76ВА" Тогда
		Результат = ДБ_ПроверитьОстаткиСчета76ВА(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ОстаткиВзаиморасчетов" Тогда
		Результат = ДБ_ПроверитьОстаткиВзаиморасчетов(СтрокаПроверки, ПараметрыОтчета);
	КонецЕсли;
	
	// Раздел "Первичные документы"
	Если СтрокаПроверки.Идентификатор = "НеПроведенныеДокументы" Тогда
		Результат = ДБ_ПроверитьНаличиеНепроведенныхДокументов(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ПоступлениеТоваровИУслуг" Тогда
		Результат = ДБ_ПроверитьПоступлениеТоваровИУслуг(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "СчетФактураПолученный" Тогда
		Результат = ДБ_ПроверитьСчетФактураПолученный(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "СчетФактураВыданный" Тогда
		Результат = ДБ_ПроверитьСчетФактураВыданный(СтрокаПроверки, ПараметрыОтчета);
	КонецЕсли;
	
	// Раздел "Справочники"
	Если СтрокаПроверки.Идентификатор = "ДублиКонтрагентов" Тогда
		Результат = ДБ_ПроверитьДублиКонтрагентов(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ДублиДоговоров" Тогда
		Результат = ДБ_ПроверитьДублиДоговоров(СтрокаПроверки, ПараметрыОтчета);
	ИначеЕсли СтрокаПроверки.Идентификатор = "ДублиНоменклатуры" Тогда
		Результат = ДБ_ПроверитьДублиНоменклатуры(СтрокаПроверки, ПараметрыОтчета);
	КонецЕсли;
#КонецВставки
...
///////////////////////////////////////////////////////////////////////////////////////
// Проверки раздела "Остатки на счетахи"

Функция ДБ_ПроверитьОтрицательныеОстаткиНаСчетахБУ(СтрокаПроверки, ПараметрыОтчета)
	...
	ДБ_ЗаполнитьРасшифровку(ТаблицаРезультата, СтрокаПроверки.Идентификатор);
	
	Возврат ТаблицаРезультата;
	
КонецФункции // ПроверитьОстаткиСчета10()

Функция ДБ_ПроверитьОстаткиСчета10(СтрокаПроверки, ПараметрыОтчета)
	
	Запрос = Новый Запрос;
	Запрос.Текст = ДБ_ТекстЗапросаДляСчетовУчетамТМЦ();
...
Функция ДБ_ПроверитьОстаткиСчета19(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета41(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета60(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета62(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета6802(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета76АВ(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиСчета76ВА(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьОстаткиВзаиморасчетов(СтрокаПроверки, ПараметрыОтчета)
...
///////////////////////////////////////////////////////////////////////////////////////
// Проверки раздела "Первичные документы"

Функция ДБ_ПроверитьНаличиеНепроведенныхДокументов(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьПоступлениеТоваровИУслуг(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьСчетФактураПолученный(СтрокаПроверки, ПараметрыОтчета)
...
Функция ДБ_ПроверитьСчетФактураВыданный(СтрокаПроверки, ПараметрыОтчета)
...
///////////////////////////////////////////////////////////////////////////////////////
// Проверки раздела "Справочникии"

Функция ДБ_ПроверитьДублиНоменклатуры(СтрокаПроверки, ПараметрыОтчета)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Строка.ДатаСоздания = ДБ_ДатаСозданияСсылки(Строка.Справочник);
...
Функция ДБ_ПроверитьДублиКонтрагентов(СтрокаПроверки, ПараметрыОтчета)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Строка.ДатаСоздания = ДБ_ДатаСозданияСсылки(Строка.Контрагент);
...
Функция ДБ_ПроверитьДублиДоговоров(СтрокаПроверки, ПараметрыОтчета)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Строка.ДатаСоздания = ДБ_ДатаСозданияСсылки(Строка.Договор);
...
Функция ДБ_ДатаСозданияСсылки(Ссылка)
...
Функция ДБ_ТекстЗапросаДляСчетовУчетамТМЦ()
...
//
Процедура ДБ_ЗаполнитьРасшифровку(ТаблицаРезультата, Идентификатор)
